...
def test_hard_flags_variaveis_e_estados(rbn_sistema: SistemaTriagemBayesiana) -> None:
    ...
    assert estados_gravidade == ["baixa", "média", "alta"], "Falha na Hard Flag 3: Nomenclatura dos estados incorreta!"
    
    # Verifica as 6 variáveis totais (5 inputs + 1 output)
    variaveis_modelo = rbn_sistema.model.nodes()
    variaveis_esperadas = [
        "IdadeAvancada", "Gravidade", "SaturacaoO2", 
        "FrequenciaCardiaca", "NivelDor", "Febre"
    ]
    assert set(variaveis_modelo) == set(variaveis_esperadas), "Falha nas Hard Flags 1 e 2: Estrutura de variáveis inválida!"
...
def test_comportamento_com_evidencias_incompletas(rbn_sistema: SistemaTriagemBayesiana) -> None:
    """
